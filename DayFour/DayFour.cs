namespace AdventKalenderAfKode2022
{
    ...
    {
        /**
        --- Day 4: Camp Cleanup ---Space needs to be cleared before the last supplies can be unloaded from the ships,
        ...
        **/
        ...
        {
            string text = System.IO.File.ReadAllText(@"DayFour\input.txt");
            List<string> lines = text.Split(new string[] { "\r\n" }, StringSplitOptions.None).ToList();

            return lines.Select((item) => item.Split(new string[] { ",", "-" }, StringSplitOptions.None).Select((item) => Int32.Parse(item)).ToList()).ToList();
        }

        public static int getRange()
        {
            List<List<int>> inputLines = readInput();
            return inputLines.Where((item) =>
            {
                var range = (item[0] < item[2] ? item[0] : item[2])..(item[1] > item[3] ? item[1] : item[3]);
                return range.Equals((item[0]..item[1])) || range.Equals((item[2]..item[3]));
            }).Count();
        }
        ...
        **/
        ...
        public static int getOverlaps()
        {
            ...
            return inputLines.Where((item) => !(item[1] < item[2] || item[3] < item[0]))
                             .Count();
        }
    }
